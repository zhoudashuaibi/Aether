...
use super::{
    ...
    sample_files_provider_catalog_key, sample_files_provider_catalog_provider,
    sample_files_proxy_node_repository, start_server, to_bytes, Arc, Body,
...
async fn gateway_executes_gemini_files_list_via_local_decision_gate_with_local_planning_only_impl()
{
    ...
    let gemini_file_mapping_repository =
        Arc::new(super::InMemoryGeminiFileMappingRepository::seed([
            super::sample_owned_file_mapping("files/owned-list-file", "user-files-list-local-123"),
            super::sample_owned_file_mapping("files/other-list-file", "user-other"),
        ]));
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
    ...
    assert_eq!(payload["files"][0]["name"], "files/owned-list-file");
    assert_eq!(
...
async fn gateway_executes_gemini_files_delete_via_local_decision_gate_with_local_planning_only_impl(
    ...
    let gemini_file_mapping_repository =
        Arc::new(super::InMemoryGeminiFileMappingRepository::seed([
            super::sample_owned_file_mapping("files/abc-123", "user-files-delete-local-123"),
        ]));
    let provider_catalog_repository = Arc::new(InMemoryProviderCatalogReadRepository::seed(
        vec![sample_files_provider_catalog_provider()],
        vec![sample_files_provider_catalog_endpoint()],
